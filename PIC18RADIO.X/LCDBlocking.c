...
//	LCDText[sizeof(LCDText)-1] = 0;
    ...
    memset(LCDText[0], ' ', sizeof(LCDText[0])-1);
    memset(LCDText[1], ' ', sizeof(LCDText[1])-1);
    
    LCDText[0][sizeof(LCDText[0])-1] = 0;
    LCDText[1][sizeof(LCDText[1])-1] = 0;
...
/*void LCDWriteLine1(BYTE line, const char *fmt, ...)
{
    char *p = LCDText[line-1];
    va_list ap;
    va_start (ap, fmt);
    vsprintf (p, fmt, ap); 
    va_end (ap);
